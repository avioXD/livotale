...
export function DashboardCharts({ overview }: DashboardChartsProps) {
  ...
  const trendData = overview.charts.clinicTrends.map((row) => ({
    ...
    fibroscan: row.avg_fibroscan != null ? Number(row.avg_fibroscan) : null,
  }));
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trendData}>
                ...
                <Line type="monotone" dataKey="fibroscan" stroke="#6366f1" strokeWidth={2} dot={false} name="Avg FibroScan kPa" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
