...
export function PatientTrendChart({ trends }: PatientTrendChartProps) {
  ...
  const data = trends.map((t) => ({
    ...
    fibroscan: t.fibroscan_kpa != null ? Number(t.fibroscan_kpa) : null,
    ...
    <div className="h-80 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data}>
          ...
          <Line type="monotone" dataKey="fibroscan" stroke="#6366f1" strokeWidth={2} dot={false} name="FibroScan kPa" />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
