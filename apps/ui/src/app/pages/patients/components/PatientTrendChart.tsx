...
export function PatientTrendChart({ trends }: PatientTrendChartProps) {
  ...
  const data = trends.map((t) => ({
    ...
    liverFibrosisScan: t.liverFibrosisScanKpa != null ? Number(t.liverFibrosisScanKpa) : null,
    ...
    <div className="h-80 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data}>
          ...
          <Line type="monotone" dataKey="liverFibrosisScan" stroke="#6366f1" strokeWidth={2} dot={false} name="Liver Fibrosis Scan kPa" />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
